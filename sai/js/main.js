...
"use strict";
var _jquery = __webpack_require__(4);var _jquery2 = _interopRequireDefault(_jquery);
var _slider = __webpack_require__(5);var _slider2 = _interopRequireDefault(_slider);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}

(0, _slider2.default)(_jquery2.default, {
    breakpoints: {
        tablet: 992,
        phone: 767 },
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });var _hammerjs = __webpack_require__(6);var _hammerjs2 = _interopRequireDefault(_hammerjs);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}

var refSlider = function refSlider(jQuery, options) {
	jQuery(document).ready(function ($) {
		...
		if (slidesWrapper.length > 0) {
			...
			var sliderNav = null;
			var controlsNav = $('.cd-slider-arrow');
			...
			var autoPlayDelay = options.autoplayInterval ? options.autoplayInterval : 5000;
			var isClicked = false;

			var breakpoints = {
				tablet: options.breakpoints.tablet ? options.breakpoints.tablet : 992,
				phone: options.breakpoints.desktop ? options.breakpoints.desktop : 767


				//detect current width and set needed images
			};changeImageset(slidesWrapper);
			...
			slidesWrapper.on('mouseenter', function () {
				if (autoplay) stopAutoplay(slidesWrapper);
			});

			slidesWrapper.on('mouseleave', function () {
				if (autoplay && !isClicked) setAutoplay(slidesWrapper, slidesNumber, autoPlayDelay);
			});

			$(document).on('click', '.cd-slider-arrow--right', function () {
				customNext();
			});

			$(document).on('click', '.cd-slider-arrow--left', function () {
				customPrev();
			});

			$(window).on('resize', function () {
				absoluteCover($('.inline-img'));

				if ($(window).width() < breakpoints.phone) stopAutoplay();else
				setAutoplay($('.cd-hero-slider'), slidesNumber, autoPlayDelay);
			});

			slidesWrapper.on('click', function () {
				stopAutoplay(slidesWrapper);
				isClicked = true;
			});
		...
		var canHover = true;
		function hover() {
			$('.cd-full-width').on('mouseover click', function (e) {
				if (canHover) {
		...
		function setTags() {
			$('ul.cd-hero-slider > li:first').addClass('first');
			$('ul.cd-hero-slider > li:last').addClass('last');
		}

		function createNavs() {
			$('.cd-hero').append('<div class="cd-slider-nav"><nav><ul></ul></nav></div>');
			sliderNav = $('.cd-slider-nav');
			for (var i = 0; i < slidesNumber; i++) {
				sliderNav.find('ul').append('<li ' + (i == 0 ? 'class="selected"' : '') + '></li>');
			}
		}

		function oneElementCheck() {
			if (slidesNumber == 1) {
				autoplay = false;
				autoPlayDelay = 0;
				controlsNav.remove();
		...
		function nextSlide(visibleSlide, container, pagination, n) {
			...
			visibleSlide.removeClass('selected from-left from-right').addClass('is-moving').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function () {
				...
				canHover = true;
				$('.cd-full-width').removeClass('hovered');
			});
		...
		function customNext() {
			...
			if (selectedPosition > slidesNumber - 1) {
				selectedPosition = 0;
				visibleSlidePosition = selectedPosition;
		...
		function setAutoplay(wrapper, length, delay) {
			...
			autoPlayId = window.setInterval(function () {
				autoplaySlider(length, visibleSlidePosition);
			}, delay);
		}

		function changeImageset(wrapper) {
			var images = wrapper.find('.inline-img');
			absoluteCover(images);

			if ($(window).width() > breakpoints.phone) {
				images.each(function (index, value) {
					$(value).attr('src', $(value).data('image-desktop'));
				});
			} else {
				images.each(function (index, value) {
					$(value).attr('src', $(value).data('image-tablet'));
				});
			}
